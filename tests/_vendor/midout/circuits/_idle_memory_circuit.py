...
from tests._vendor.midout.circuits.steps._measure_y_transition_round import make_y_transition_round_nesw_xzxz_to_xzzx
from tests._vendor.midout.circuits.steps._patches import make_xtop_qubit_patch, make_ztop_yboundary_patch
...
def make_idle_memory_with_magic_before_and_after(
        *,
        distance: int,
        memory_rounds: int,
        basis: str,
    ...
    qubit_patch = make_xtop_qubit_patch(distance=distance)
    obs_x = gen.PauliString({q*1j: 'X' for q in range(distance)})
    obs_z = gen.PauliString({q: 'Z' for q in range(distance)})
    obs_y = obs_x * obs_z
    obs = {
        'X': obs_x,
        'Y': obs_y,
    ...
    memory_round = gen.standard_surface_code_chunk(qubit_patch, obs=obs)

    return [
        memory_round.magic_init_chunk(),
        memory_round.with_repetitions(memory_rounds),
        memory_round.magic_end_chunk(),
    ]
